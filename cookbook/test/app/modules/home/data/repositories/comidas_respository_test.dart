...
class MockComidasDatasource extends Mock implements IComidasDatasource {}

void main() {
  group("Comidas repository", () {
    late MockComidasDatasource comidasDatasource;
    late ComidasRepository comidasRespository;
    setUp(() {
      comidasDatasource = MockComidasDatasource();
      comidasRespository =
          ComidasRepository(comidasDatasource: comidasDatasource);
    });

    test("Deve retornar um SuccessComidasState quando não houver excessões",
        () async {
      when(() => comidasDatasource.get("Seafood"))
          .thenAnswer((invocation) async => data);
      final result = await comidasRespository.get("Seafood");
      expect(result, isA<SuccessComidaState>());
      if (result is SuccessComidaState) {
        List<Comida> comidas = result.comidas;
        expect(comidas, isNotEmpty);
      }
    });
