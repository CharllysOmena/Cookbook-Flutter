...
class MockCategoriaDatasource extends Mock implements ICategoriaDatasource {}

void main() {
  group("Categoria repository", () {
    late MockCategoriaDatasource categoriaDatasource;
    late CategoriaRepository categoriaRespository;
    setUp(() {
      categoriaDatasource = MockCategoriaDatasource();
      categoriaRespository =
          CategoriaRepository(categoriaDatasource: categoriaDatasource);
    });

    test("Deve retornar um SuccessCategoriaState quando não houver excessões",
        () async {
      when(() => categoriaDatasource.get())
          .thenAnswer((invocation) async => data);
      final result = await categoriaRespository.get();
      expect(result, isA<SuccessCategoriaState>());
      if (result is SuccessCategoriaState) {
        List<Categoria> categorias = result.categorias;
